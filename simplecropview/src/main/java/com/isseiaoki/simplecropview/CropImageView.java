    ...
    private void drawOverlay(Canvas canvas) {
        ...
        if (!mIsAnimating) {
            if ((mCropMode == CropMode.CIRCLE || mCropMode == CropMode.CIRCLE_SQUARE)) {
                ...
                float frameSize = Math.min(frameWidth, frameHeight);
                float centerX = mFrameRect.centerX();
                float centerY = mFrameRect.centerY();
                ...
                float offsetX = centerX - (starBounds.left + starBounds.right) / 2; // 计算水平方向上的偏移量
                float offsetY = centerY - (starBounds.top + starBounds.bottom) / 2; // 计算垂直方向上的偏移量
